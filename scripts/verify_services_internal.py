import asyncio
import json
import sys
import time
from datetime import datetime

from fastapi.testclient import TestClient

# Import Apps directly (Internal Mode)
from examples.extractor_service.main import app as extractor_app
from examples.job_service.main import app as job_app

# Create Clients
extractor_client = TestClient(extractor_app)
job_client = TestClient(job_app)

BATCH_FILE = "examples/fixtures/batch_urls.txt"
OUTPUT_FILE = "service_test_outcome.txt"

async def log(f, message, console=True):
    timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    line = f"[{timestamp}] {message}"
    if console:
        print(line)
    f.write(line + "\n")
    f.flush()

async def run_internal_test():
    with open(OUTPUT_FILE, "w", encoding="utf-8") as f:
        await log(f, "=== ORGA M6.1 Service Quality Assessment (Internal Simulation Mode) ===\n")
        await log(f, "Note: Running via TestClient (direct code execution), bypassing Docker network.\n")
        
        # 1. Health Check
        await log(f, "--- Phase 1: Service Health Check ---")
        try:
            resp_ext = extractor_client.get("/health")
            await log(f, f"Extractor Service: {resp_ext.status_code} | {resp_ext.json()}")
            
            resp_job = job_client.get("/health")
            await log(f, f"Job Service:       {resp_job.status_code} | {resp_job.json()}")
        except Exception as e:
            await log(f, f"FATAL: Internal app instantiation failed. {e!s}")
            return

        # Read URLs
        with open(BATCH_FILE) as bf:
            urls = [line.strip() for line in bf if line.strip()]
        await log(f, f"\nLoaded {len(urls)} Validation Targets from {BATCH_FILE}")

        # 2. Detailed Single Extraction Test (CHEO Focus)
        target_url = "https://www.cheo.on.ca"
        await log(f, "\n--- Phase 2: Deep-Dive Single Extraction (Stability Probe) ---")
        await log(f, f"Target: {target_url}")
        
        start_time = time.time()
        try:
            # TestClient is synchronous but calls the async app
            resp = extractor_client.post(
                "/extract", 
                json={"url": target_url}
            )
            duration = time.time() - start_time
            
            if resp.status_code == 200:
                data = resp.json()
                await log(f, f"✅ SUCCESS (Time: {duration:.2f}s)")
                
                # Extract Quality Metrics
                name = data.get("name", "N/A")
                locs = data.get("locations", [])
                phones = data.get("phones", [])
                debug = data.get("debug_info", {})
                
                await log(f, f"  Name: {name}")
                await log(f, f"  Locations: {len(locs)}")
                await log(f, f"  Phones:    {len(phones)}")
                
                if debug:
                    await log(f, "  [Debug Metrics]:")
                    if "classification_debug" in debug:
                        await log(f, f"    - Classification Steps: {len(debug['classification_debug'])}")
                    if "filtered_social_links" in debug:
                        await log(f, f"    - Filtered Socials: {len(debug['filtered_social_links'])}")
                
                snippet = json.dumps(data, indent=2)
                await log(f, "\n  [Full Response Dump (First 2000 chars)]:")
                await log(f, snippet[:2000] + "\n... (truncated)", console=False)
            else:
                await log(f, f"❌ FAILED: {resp.status_code}")
                await log(f, f"  Error: {resp.text}")
        except Exception as e:
            await log(f, f"❌ EXCEPTION: {e!s}")

        # 3. Batch Job Processing
        await log(f, "\n--- Phase 3: Batch Processing Stress Test ---")
        job_id = None
        
        # Submit
        try:
            resp = job_client.post("/jobs", json={"urls": urls})
            if resp.status_code == 202:
                job_id = resp.json()["job_id"]
                await log(f, f"Job Submitted Successfully. ID: {job_id}")
            else:
                await log(f, f"Submission Failed: {resp.text}")
                return
        except Exception as e:
            await log(f, f"Submission Error: {e!s}")
            return

        # Poll
        # NOTE: In TestClient/Starlette, BackgroundTasks typically run *after* the response is sent
        # but within the same thread/context. By the time .post() returns, tasks might already be done
        # or scheduled.
        # However, for `async def` endpoints, it might be tricky.
        # But usually TestClient handles it. Let's see.
        
        await log(f, "Polling job status...")
        start_poll = time.time()
        
        # We simulate polling, but since TestClient is sync, the job might finish instantly
        # if the background task execution strategy allows it.
        # If it doesn't run background tasks automatically, we might need to manually trigger.
        # But FastAPI TestClient (Starlette) DOES run background tasks.
        
        while True:
            resp = job_client.get(f"/jobs/{job_id}")
            data = resp.json()
            status = data["status"]
            elapsed = time.time() - start_poll
            
            sys.stdout.write(f"\rTime: {elapsed:.1f}s | Status: {status} | Done: {len(data.get('results', []))} | Failed: {len(data.get('errors', []))}")
            sys.stdout.flush()
            
            if status in ["completed", "failed"]:
                print("") 
                await log(f, f"\nJob Completed in {elapsed:.2f}s")
                
                results = data.get("results", [])
                errors = data.get("errors", [])
                
                await log(f, "\n=== QUALITY ANALYSIS REPORT ===")
                await log(f, f"Total Targets: {len(urls)}")
                await log(f, f"Success Rate:  {len(results)/len(urls)*100:.1f}% ({len(results)}/{len(urls)})")
                
                if errors:
                    await log(f, "\n[FAILURE LOG]")
                    for err in errors:
                        await log(f, f"❌ {err}")
                else:
                    await log(f, "\n[FAILURE LOG]\n  (None - Perfect Run)")

                await log(f, "\n[RESULT QUALITY SAMPLE]")
                total_locs = sum(len(r.get("locations", [])) for r in results)
                total_phones = sum(len(r.get("phones", [])) for r in results)
                await log(f, f"Total Extracted Locations: {total_locs}")
                await log(f, f"Total Extracted Phones:    {total_phones}")
                
                await log(f, "\n--- Detailed Profile Dumps ---", console=False)
                for i, res in enumerate(results):
                    name = res.get("name") or "Unknown"
                    await log(f, f"\n#{i+1}: {name}", console=False)
                    debug_section = res.get("debug_info", {})
                    warnings = res.get("warnings", [])
                    if warnings:
                        await log(f, f"  Warnings: {[w['code'] for w in warnings]}", console=False)
                    await log(f, json.dumps(res, indent=2), console=False)
                
                break
            
            # Since TestClient runs synchronously, if status isn't completed immediately,
            # it implies tasks run in a thread? 
            # Or maybe we need to wait.
            await asyncio.sleep(1)
            if elapsed > 120: # Timeout
                await log(f, "\nTIMEOUT waiting for job completion.")
                break

if __name__ == "__main__":
    asyncio.run(run_internal_test())
